...
class Catalog_Engine {
	...
	public function ajax_filter() {
		...
		wp_send_json_success(
			[
				'html'       => $this->grid_html( $query, $columns, (bool) $params['append'] ),
			...
			if ( ! $is_archive ) {
				...
				<div class="hkdev-cat-results">
					...
					<div class="hkdev-cat-grid hkdev-shop-grid hkdev-columns-<?php echo esc_attr( $columns ); ?>">
						<?php echo $this->grid_html( $query, $columns, false ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</div>
	 ...
	 * @return string
	 */
	private function grid_html( $query, $columns, $append ) {
		...
		while ( $query->have_posts() ) {
			$query->the_post();
			$engine->render_single_product_card( get_the_ID(), 0, false );
		}
		wp_reset_postdata();
		return ob_get_clean();
	}
