...
class CatalogEngine {
	...
	public function build_query_args( $params, $per_page ) {
		...
		// Slug form with include_children, exactly as ShopEngine builds the same
		...
		// JOIN, so those products stay listed - the same reason ShopEngine's
		...
		switch ( $params['sort'] ) {
			...
			case 'title':
				...
				break;
			case 'oldest':
				// ID ties the ordering to ShopEngine's: products often share a
	...
	public function ajax_filter() {
		...
		$button = \HkdevShopElements\Includes\Core\ShopEngine::normalize_button_atts( $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.Missing

		wp_send_json_success(
			[
				'html'       => $this->grid_html( $query, $columns, (bool) $params['append'], $hover, $image, $button ),
	...
	public function render( $atts = [], $is_archive = false ) {
		...
		$button     = \HkdevShopElements\Includes\Core\ShopEngine::normalize_button_atts( $atts );
			...
			if ( ! $is_archive ) {
				...
				<div class="hkdev-cat-results">
					...
					<div class="hkdev-cat-grid hkdev-shop-grid hkdev-columns-<?php echo esc_attr( $columns ); ?>">
						<?php echo $this->grid_html( $query, $columns, false, $show_hover, $image, $button ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</div>
	 ...
	 * @param array     $button   Normalized Buy Now attributes.
	 * @return string
	 */
	private function grid_html( $query, $columns, $append, $hover = true, $image = [], $button = [] ) {
		...
		$button = \HkdevShopElements\Includes\Core\ShopEngine::normalize_button_atts( $button );
