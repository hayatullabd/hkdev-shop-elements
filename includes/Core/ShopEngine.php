...
class ShopEngine {
	 ...
	 * @param array  $card_opts        Extra card options (Buy Now text/action).
	 * @return void
	 */
	public function render_single_product_card( $post_id, $trending_days = 0, $is_carousel = false, $image_size = 'woocommerce_thumbnail', $show_hover_image = false, $image_size_mode = 'preset', $custom_img_w = 0, $custom_img_h = 0, $card_opts = [] ) {
		global $product;
			...
			<div class="hkdev-content-box">
				...
				<div class="hkdev-footer-actions">
					<?php if ( 'instock' === $stock_status ) : ?>
						<?php echo $this->render_card_buy_button( $post_id, $permalink, $checkout_url, $is_variable, $card_opts ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<?php else : ?>
	...
	/**
	 * Product card Buy Now / Add to Cart / View Product button.
	 *
	 * @param int    $post_id       Product ID.
	 ...
	 * @param array  $card_opts     Raw widget / request options.
	 * @return string
	 */
	protected function render_card_buy_button( $post_id, $permalink, $checkout_url, $is_variable, $card_opts = [] ) {
		$button = self::normalize_button_atts( $card_opts );
		if ( 'yes' !== $button['show_buy_now'] ) {
			return '';
		}

		$text   = '' !== $button['buy_now_text'] ? $button['buy_now_text'] : __( 'Buy Now', 'hkdev-shop-elements' );
		$action = $button['buy_now_action'];
		$after  = $button['buy_now_after'];
		$icon   = 'checkout' === $action ? 'fa-bolt' : ( 'add_to_cart' === $action ? 'fa-cart-plus' : 'fa-arrow-right' );
		...
		<div class="hkdev-action-group">
			<?php if ( 'product' === $action ) : ?>
				<a href="<?php echo esc_url( $permalink ); ?>"
					...
					data-btn-action="product">
					<?php if ( 'yes' === $button['buy_now_icon'] ) : ?><i class="fa-solid <?php echo esc_attr( $icon ); ?>" aria-hidden="true"></i> <?php endif; ?><?php echo esc_html( $text ); ?>
				</a>
			<?php elseif ( $is_variable ) : ?>
				<button type="button"
					...
					data-btn-action="<?php echo esc_attr( $action ); ?>"
					data-btn-after="<?php echo esc_attr( $after ); ?>">
					<?php if ( 'yes' === $button['buy_now_icon'] ) : ?><i class="fa-solid <?php echo esc_attr( $icon ); ?>" aria-hidden="true"></i> <?php endif; ?><?php echo esc_html( $text ); ?>
				</button>
			<?php else : ?>
				<button type="button"
	...
	protected function listing_params_from_request( $with_paged = false ) {
		...
		$params = array_merge( $params, self::normalize_image_atts( $_POST ), self::normalize_button_atts( $_POST ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function ajax_filter_products_callback() {
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				$query->the_post();
				$this->render_single_product_card( get_the_ID(), $params['days'], ( 'carousel' === $style ), $params['image_size'], 'yes' === $params['hover_img'], $params['image_size_mode'], $params['custom_img_w'], $params['custom_img_h'], $params );
			}
			wp_reset_postdata();
		} else {
	...
	/**
	 * Normalize Buy Now button attributes shared by product widgets.
	 *
	 * @param array $atts Raw widget or request attributes.
	 * @return array<string,string>
	 */
	public static function normalize_button_atts( $atts ) {
		$atts   = is_array( $atts ) ? $atts : [];
		$action = sanitize_key( isset( $atts['buy_now_action'] ) ? (string) $atts['buy_now_action'] : 'checkout' );
		$after  = sanitize_key( isset( $atts['buy_now_after'] ) ? (string) $atts['buy_now_after'] : 'auto' );
		...
		return [
			'show_buy_now'   => ( isset( $atts['show_buy_now'] ) && 'no' === $atts['show_buy_now'] ) ? 'no' : 'yes',
			'buy_now_text'   => isset( $atts['buy_now_text'] ) ? sanitize_text_field( (string) $atts['buy_now_text'] ) : '',
			'buy_now_icon'   => ( isset( $atts['buy_now_icon'] ) && 'no' === $atts['buy_now_icon'] ) ? 'no' : 'yes',
	...
	public function master_shop_shortcode( $atts = [] ) {
		...
		$design       = self::normalize_design_atts( $atts );
		$image        = self::normalize_image_atts( $atts );
		$button       = self::normalize_button_atts( $atts );
		...
		<div class="<?php echo esc_attr( $wrapper_class ); ?>" id="<?php echo esc_attr( $unique_id ); ?>"
			 ...
			 data-show-buy-now="<?php echo esc_attr( $button['show_buy_now'] ); ?>"
			 data-buy-now-text="<?php echo esc_attr( $button['buy_now_text'] ); ?>"
			 data-buy-now-icon="<?php echo esc_attr( $button['buy_now_icon'] ); ?>"
			 data-buy-now-action="<?php echo esc_attr( $button['buy_now_action'] ); ?>"
			 data-buy-now-after="<?php echo esc_attr( $button['buy_now_after'] ); ?>"
					...
					<div class="swiper hkdev-swiper-container hkdev-loading-carousel">
						<div class="swiper-wrapper hkdev-shop-grid">
							<?php if ( $query->have_posts() ) : ?>
								...
								while ( $query->have_posts() ) {
									$query->the_post();
									$this->render_single_product_card( get_the_ID(), (int) $atts['days'], true, $atts['image_size'], $show_hover, $img_size_mode, $custom_img_w, $custom_img_h, $button );
								}
								wp_reset_postdata();
								?>
							<?php else : ?>
				...
				<?php else : ?>
					<div class="hkdev-shop-grid hkdev-columns-<?php echo esc_attr( $cols_desktop ); ?>">
						<?php if ( $query->have_posts() ) : ?>
							...
							while ( $query->have_posts() ) {
								$query->the_post();
								$this->render_single_product_card( get_the_ID(), (int) $atts['days'], false, $atts['image_size'], $show_hover, $img_size_mode, $custom_img_w, $custom_img_h, $button );
							}
							wp_reset_postdata();
							?>
						<?php else : ?>
